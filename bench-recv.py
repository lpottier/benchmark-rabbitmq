...
def consume_worker(config, cacert, exchange, queue, routing_key, process_index, stats_dict, max_messages, timeout, qos, verbose):
    ...
    if exchange != '':
        ...
        if verbose:
            print(f"[Recv {process_index}] Bound queue '{queue_name}' to exchange '{exchange}' with routing key '{routing_key}'")
...
def main():
    ...
    for i in range(args.processes):
        proc = Process(target=consume_worker,
                       args=(config, cacert, args.exchange, args.queue, args.routing_key, i, stats_dict, messages_per_proc, args.timeout, args.qos, args.verbose))
        proc.start()
